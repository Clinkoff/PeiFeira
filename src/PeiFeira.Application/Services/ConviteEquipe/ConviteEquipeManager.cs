...
public class ConviteEquipeManager : IConviteEquipeManager
{
    ...
    {
        var perfilAluno = await _unitOfWork.PerfisAluno.GetByIdAsync(perfilAlunoId);
        if (perfilAluno == null)
        {
            return Enumerable.Empty<ConviteEquipeResponse>();
        }

        var convites = await _unitOfWork.ConvitesEquipe.GetPendentesAsync(perfilAluno.UsuarioId);
    ...
    public async Task<int> ContarConvitesPendentesAsync(Guid perfilAlunoId)
    {
        ...
        return await _unitOfWork.ConvitesEquipe.CountConvitesPendentesAsync(perfilAluno.UsuarioId);
    }
