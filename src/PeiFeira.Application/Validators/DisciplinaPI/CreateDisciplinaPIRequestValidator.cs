...
public class CreateDisciplinaPIRequestValidator : BaseValidator<CreateDisciplinaPIRequest>
{
    public CreateDisciplinaPIRequestValidator()
    {
        RuleFor(d => d.SemestreId)
            .NotEmpty()
            .WithMessage("Semestre é obrigatório")
            .Must(BeValidGuid)
            .WithMessage("Semestre ID inválido");

        RuleFor(d => d.PerfilProfessorId)
            .NotEmpty()
            .WithMessage("Professor é obrigatório")
            .Must(BeValidGuid)
            .WithMessage("Professor ID inválido");
